    ...
    private void newTaskButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_newTaskButtonActionPerformed
        ...
        if (newToDotxt != null && newToDotxt.length() > 0)
        {
            try {
                Connection con =  database.ConnectDB();
                Statement stmt = con.createStatement();
                String query = "SELECT REFERENCE FROM TASKS WHERE REFERENCE = (SELECT MAX(REFERENCE) FROM TASKS) AND NAME = '"+name+"'";
                ResultSet rs = stmt.executeQuery(query);

                int referenceNum = Integer.parseInt(rs.getString("reference"));
                referenceNum++;              
                
                query = "INSERT INTO TASKS VALUES('"+referenceNum+"','"+name+"','"+newToDotxt+"','0')";
                stmt.execute(query);
            }   
            catch(SQLException e)
            {
